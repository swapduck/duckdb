-- BUILD SIDE (Right side of JOIN)
-- Small total row count, but one partition is huge
CREATE TABLE t2 AS
SELECT
    1 AS join_key,
...
FROM range(2000000) t(i) -- This partition alone is ~200MB (exceeds 100MB limit)
UNION ALL
SELECT
    i + 10 AS join_key,
    hash(i)::UBIGINT AS b1, hash(i+1)::UBIGINT AS b2,
